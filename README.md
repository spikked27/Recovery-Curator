...
- An in-app AI conversation that receives bounded catalog summaries, representative filenames, and the context you provide. It can ask follow-up questions and propose at most three broad origin, sensitivity, or collection rules per reply.
- AI proposals show their estimated reach and require one confirmation. They label the catalog only; the model cannot reconstruct folders, delete content, edit metadata, or run an export.
...
The normal AI workflow is a conversation inside **Curate**. Each turn receives a bounded statistical summary, the context you entered, and a rotating sample of representative filenames—not the entire catalog. The assistant can ask clarifying questions and propose up to three literal substring rules for broad origin, sensitivity, or collection labels. Each proposal displays how many files it would affect and remains inert until confirmed. It cannot create original-folder claims, choose deletions, suppress known-good safeguards, change metadata, or run the build.
